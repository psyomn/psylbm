...
void psy_lbm_free_server(psy_lbm_server_t *_s)
{
	free(_s->hostname);
	free(_s);
}

void psy_lbm_server_listen(psy_lbm_server_t *_server)
{
	int fd;
...
void psy_lbm_print_server_info(psy_lbm_server_t *_server)
{
	printf("%s: %d\n" "%s: %s\n" "%s: %s\n",
...
const char *psy_lbm_server_status(psy_lbm_server_t *_server)
{
	...
	for (index = 0; index < size; --index)
		if (psy_lbm_statuses[index].id ==
		    _server->status) return psy_lbm_statuses[index].status;
